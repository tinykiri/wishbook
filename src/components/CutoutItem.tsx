...
export default function CutoutItem({ item, onDelete, onUpdate, canvasScale = 1, readOnly = false, boardWidth }: ItemProps) {
  ...
  const clipPath = useMemo(() => generateClipPath(localSeed), [localSeed]);
  ...
  }, [drawings, currentPath, scale, localSeed, penColor, eraserSize, penWidth]);
                ...
                <div className="flex items-center gap-2">
                  ...
                  {penColor !== 'erase' && (
                    <>
                      ...
                      <div className="relative w-5 h-5 rounded-full border border-slate-200 overflow-hidden bg-gradient-to-br from-red-500 via-green-500 to-blue-500 hover:scale-110 transition-transform"><input type="color" value={penColor} onChange={(e) => setPenColor(e.target.value)} className="absolute inset-0 opacity-0 w-full h-full cursor-pointer" /></div>
                    </>
                  )}
                  <div className="w-[1px] bg-slate-200 h-4 mx-1"></div>
